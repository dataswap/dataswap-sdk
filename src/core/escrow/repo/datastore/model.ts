/*******************************************************************************
 *   (c) 2023 dataswap
 *
 *  Licensed under either the MIT License (the "MIT License") or the Apache License, Version 2.0
 *  (the "Apache License"). You may not use this file except in compliance with one of these
 *  licenses. You may obtain a copy of the MIT License at
 *
 *      https://opensource.org/licenses/MIT
 *
 *  Or the Apache License, Version 2.0 at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the MIT License or the Apache License for the specific language governing permissions and
 *  limitations under the respective licenses.
 ********************************************************************************/

import mongoose, { Schema, Document } from "mongoose"
import { ValueFields } from "@unipackage/utils"

import { Escrow } from "../../types"

/**
 * Interface representing a EscrowDocument, extending Escrow and Document.
 * @interface
 */
interface EscrowDocument extends ValueFields<Escrow>, Document {}

/**
 * Schema definition for the Escrow collection.
 * @constant
 */
const EscrowSchema = new Schema<EscrowDocument>({
    type: {
        type: Number,
        required: [true, "Please provide the type"],
    },
    owner: {
        type: String,
        required: [true, "Please provide the owner"],
    },
    datasetId: {
        type: Number,
        required: [true, "Please provide the datasetId"],
    },
    matchingId: {
        type: Number,
        required: [true, "Please provide the matchingId"],
    },
    beneficiary: {
        type: String,
        required: [true, "Please provide the beneficiary"],
    },
    amount: {
        type: BigInt,
        required: [true, "Please provide the amount"],
    },
})

/**
 * Model for the EscrowModel collection.
 * @constant
 */
const EscrowModel =
    mongoose.models.Escrow ||
    mongoose.model<EscrowDocument>("Escrow", EscrowSchema)

export { EscrowModel }
export type { EscrowDocument }
