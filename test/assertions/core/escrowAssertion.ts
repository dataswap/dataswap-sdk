...
export class EscrowAssertion implements IEscrowAssertion {
    ...
    async getOwnerFundAssertion(
        ...
        assert.isTrue(
            equal(expectFund, fund.data),
            "Owner fund should be expect fund"
        )
    }
     ...
     * @returns {Promise<void>}
     */
    async getBeneficiariesListAssertion(
        type: EscrowType,
        owner: string,
        id: number,
        expectBeneficiaries: string[]
    ) {
        let beneficiaries = await handleEvmError(
            this.escrow.getBeneficiariesList(type, owner, id)
        )
        assert.isTrue(
            equal(
                beneficiaries.data.slice().sort(),
                expectBeneficiaries.slice().sort()
            ),
            "Beneficiaries List should be expect beneficiaries list"
        )
    }
     ...
     * @returns {Promise<void>}
     */
    async getBeneficiaryFundAssertion(
        type: EscrowType,
        owner: string,
        id: number,
        beneficiary: string,
        ...
        let fund = await handleEvmError(
            this.escrow.getBeneficiaryFund(type, owner, id, beneficiary)
        )
        expectFund.createdBlockNumber = fund.data.createdBlockNumber // skip createdBlockNumber
        assert.isTrue(
            equal(expectFund, fund.data),
            "Beneficiary fund should be expect fund"
        )
    }
    ...
    async collateralAssertion(
        ...
        await handleEvmError(
            this.escrow.collateral(type, owner, id, amount, {
                value: amount,
            })
        )
    ...
    async paymentAssertion(
        ...
        this.escrow
            ...
            .setDefault(process.env.DATASWAP_GOVERNANCE as string)
        await handleEvmError(
            this.escrow.payment(type, owner, id, amount, {
                value: amount,
            })
        )
     ...
     * @returns {Promise<void>}
     */
    async paymentSingleBeneficiaryAssertion(
        type: EscrowType,
        owner: string,
        id: number,
        beneficiary: string,
        ...
        let beneficiaryFund = await handleEvmError(
            this.escrow.getBeneficiaryFund(type, owner, id, beneficiary)
        )
        ...
        this.escrow
            ...
            .setDefault(process.env.DATASWAP_GOVERNANCE as string)
        await handleEvmError(
            this.escrow.paymentSingleBeneficiary(
                type,
                owner,
                id,
                beneficiary,
        ...
        await Promise.all([
            ...
            this.getBeneficiaryFundAssertion(
                type,
                owner,
                id,
                beneficiary,
                expectBeneficiaryFund
            ),
            this.getBeneficiariesListAssertion(type, owner, id, [
                process.env.DATASWAP_GOVERNANCE as string,
                beneficiary,
            ]),
        ])
     ...
     * @returns {Promise<void>}
     */
    async paymentTransferAssertion(
        type: EscrowType,
        owner: string,
        id: number
    ) {
        ...
        this.escrow
            ...
            .setDefault(process.env.DATASWAP_GOVERNANCE as string)
        await handleEvmError(
            this.escrow.paymentTransfer(type, owner, id, amount)
        )
